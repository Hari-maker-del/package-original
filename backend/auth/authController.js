    ...
    if (supabaseConfigured()) {
      ...
      try {
        ...
        if (error?.status === 400 || error?.status === 401) {
          return res.status(401).json({ message: "Invalid email or password", detail: error.message });
        }
        throw error;
      }

      if (!data.user) return res.status(401).json({ message: "Invalid email or password" });
