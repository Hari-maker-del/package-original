...
const databaseErrorResponse = (res, error, operation) => {
  console.error(`${operation} error:`, error);
  return res.status(500).json({
    message: "Database operation failed",
    code: error?.code || "UNKNOWN",
    detail: String(error?.message || "Unknown database error").slice(0, 300),
  });
};
...
const signup = async (req, res) => {
  try {
    ...
    return databaseErrorResponse(res, error, "Signup");
  }
};
...
const login = async (req, res) => {
  try {
    ...
    return databaseErrorResponse(res, error, "Login");
  }
};
